...
public:
	...
	struct Material
	{
		...
		SmartPtr<PluggableShader> shader;
                //SmartPtr<DefaultPhongShader> shader;

		Material(){}
		Material(std::string _name) : name(_name), diffuseCoeff(0, 0, 0, 0)
		{}

                void setShaderAndKeepValues(SmartPtr<DefaultPhongShader> newShader){
                    DefaultPhongShader* temp = (DefaultPhongShader*) shader.data();
                        newShader->diffuseCoef = temp->diffuseCoef;
			newShader->ambientCoef = temp->ambientCoef;
			newShader->specularCoef = temp->specularCoef;
			newShader->specularExponent = temp->specularExponent;

			newShader->diffuseTexture = temp->diffuseTexture;
			newShader->specularTexture = temp->specularTexture;
			newShader->ambientTexture = temp->ambientTexture;
                        newShader->bumpTexture = temp->bumpTexture;
                        newShader->bumpIntensity = temp->bumpIntensity;
