...
struct Vector {
    ...
    static const Vector lerp(const Vector &_v1, const Vector &_v2, const Vector &_v3, float _u, float _v) {
        return Vector(
...
struct float2 {
    ...
    _DEF_SCALAR_OP2_SYM( /, float);
        ...
        static const float2 lerp(const float2 &_f1, const float2 &_f2, const float2 &_f3, float _u, float _v) {
        return float2(
                _f1.x * _u + _f2.x * _v + _f3.x * (1 - _u - _v),
                _f1.y * _u + _f2.y * _v + _f3.y * (1 - _u - _v)
                );
    }
};
