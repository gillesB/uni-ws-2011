    ...
    void readMtlLib(const std::string &_fileName, LWObject::t_materialVector &_matVector, const t_materialMap &_matMap) {
        ...
        while (!matInput.eof()) {
            ...
            if (_strnicmp(cmd, "newmtl", 6) == 0) {
                ...
                switch (coeffType) {
                    case 'd':
                        _matVector[curMtl].diffuseCoeff = color;
                        break;
                    case 'a':
                        _matVector[curMtl].ambientCoeff = color;
                        break;
                    case 's':
                        _matVector[curMtl].specularCoeff = color;
                        break;
                }
...
parse_err_found:
	...
	if(_createDefautShaders)
	{
		...
		{
                    //create by default a DefaultPhongShader
			SmartPtr<DefaultPhongShader> shader = new DefaultPhongShader;
			it->shader = shader;
			...
			shader->diffuseTexture = it->diffuseTexture;
			shader->specularTexture = it->specularTexture;
