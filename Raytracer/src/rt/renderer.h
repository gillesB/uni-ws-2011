...
public:
    ...
    /**
     * Loop through all pixels in the scene, take the ray from the camera,
     * forward it to the getRadiance function of the integrator, to determine
     * the pixel color.
     */
    void render() {
...
#pragma omp for schedule(dynamic, 10)
            ...
            for (int y = 0; y < (int) target->height(); y++) {
                std::cout << "Rendering pixel row y=" << y << "of "<< height << " rows" << std::endl;
